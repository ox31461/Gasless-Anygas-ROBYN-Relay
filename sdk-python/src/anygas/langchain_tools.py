...
def anygas_tools(client: Optional[AnyGas] = None):
    ...
    @tool
    def anygas_quote(src_chain: int, dst_chain: int, amount_usd: float) -> str:
        """Quote a gasless cross-chain route: moving amount_usd from src_chain to dst_chain."""
        return json.dumps(ag.quote(src_chain, dst_chain, amount_usd))
